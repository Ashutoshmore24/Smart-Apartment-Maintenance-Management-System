...
const TechnicianDashboard = () => {
    ...
    return (
        <div className="max-w-4xl px-6 py-8 mx-auto">
            <h1 className="mb-2 text-2xl font-bold text-gray-900">Technician Dashboard</h1>
            <p className="mb-6 text-gray-600">Welcome, {user?.name}. Here are your assigned tasks.</p>

            {requests.length === 0 ? (
                <div className="p-8 text-center text-gray-500 bg-white border border-gray-300 border-dashed rounded-lg">
                    No tasks assigned to you yet.
                </div>
            ) : (
                <div className="space-y-4">
                    {requests.map(req => (
                        <div key={req.request_id} className="flex items-center justify-between p-6 bg-white border border-gray-100 rounded-lg shadow-sm">
                            <div>
                                <div className="flex items-center gap-3 mb-2">
                                    <h3 className="text-lg font-semibold text-gray-800">{req.request_type}</h3>
                                    <StatusBadge status={req.status} />
                                </div>
                                <p className="mb-1 text-sm text-gray-600">Resident: <strong>{req.resident_name}</strong></p>
                                <p className="text-sm text-gray-500">{req.description || 'No additional details'}</p>
                            </div>
